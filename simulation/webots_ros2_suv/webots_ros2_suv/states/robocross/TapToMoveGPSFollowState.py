...
class TapToMoveGPSFollowState(AbstractState):
    ...
    def on_event(self, event, world_model=None):
        ...
        self.logi(str(self.local_path))

        if self.__cur_path_point == 0:
            self.current_direction = self.get_cur_directrion()
        elif self.current_direction != self.self.get_cur_directrion():
            self.timer = 40
            self.allow_timer = True
            self.current_direction = self.self.get_cur_directrion()
