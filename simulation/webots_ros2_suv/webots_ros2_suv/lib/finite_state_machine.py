...
class FiniteStateMachine:
    ...
    def detect_path(self, py_filename, full_path):
        for p in sys.path:
            fp = f'{p}{full_path}'
            #self.node._logger.info(f'{full_path}{py_filename}')
    ...
    def load_states(self, state_paths):
        ...
        for state_name, path in state_paths.items():
            if not module_path:
                module_path = self.detect_path(path, full_path = '/webots_ros2_suv/states/')
    ...
    def on_event(self, event):
        new_states = {}
        for key, state in self.current_states.items():
            next_states = state.on_event(event, None)
        ...
        if len(new_states) > 0:
            ...
            #self.node._logger.info(f"Transitioned to {[state.__qualname__.lower().replace('state', '') for s in self.current_states]}")
