...
class NodeGpsReader(Node):
    def __init__(self):
        try:
            super().__init__('node_gps_reader')
            ...
            self.__gps = GPSReader("/dev/ttyUSB0", 3000000)
            self.gps_read_timer()
        except  Exception as err:
            print(f'{str(err)}')


    def gps_read_timer(self):
        self.__gps.read_data()
        self._logger.info(f'lat: {self.__gps.lat_dec}, lon: {self.__gps.lon_dec}')
        threading.Timer(1, self.gps_read_timer).start()
