...
int BLDCMotor::getPosition() {
  if (fabs(shaft_angle - low) < threshold) {
    return GM_LOW;
  }
  if (fabs(shaft_angle - high) < threshold) {
    return GM_HIGH;
  }
  if (fabs(shaft_angle - middle) < threshold) {
    return GM_MIDDLE;
  }
  return GM_UNDEFINED;
}

void BLDCMotor::setPostions(float low_position, float middle_position, float high_position, float threshold_value) {
  low = low_position;
