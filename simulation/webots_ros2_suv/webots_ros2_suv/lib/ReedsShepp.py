...
def calc_optimal_path(sx, sy, syaw, gx, gy, gyaw, maxc, step_size=STEP_SIZE):
    ...
    minL = paths[0].L
    minL_without_backwards = paths[0].L
    mini = 0
    mini_without_backwards = -1

    for i in range(len(paths)):
        if paths[i].L <= minL_without_backwards and 1 not in paths[i].directions:
            minL_without_backwards, mini_without_backwards = paths[i].L, i
        if paths[i].L <= minL:
            minL, mini = paths[i].L, i

    if mini_without_backwards != -1:
        return paths[mini_without_backwards]
